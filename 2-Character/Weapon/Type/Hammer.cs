...
public class Hammer : IWeapon<PlayerWeapon>
{
    ...
    public void WeaponSetting(PlayerWeapon Weapon)
    {
        ...
        {
            if (wp.name == ItemColumn.EWeapon.Hammer.ToString())
            {
                ...
                Weapon.WeaponCol = wp.GetComponentInChildren<BoxCollider>();
                Weapon.AruaPrefab = wp.GetComponentInChildren<PSMeshRendererUpdater>();
                Weapon.SkilPrefab = Weapon.EffectHolder.SerachPlayerEffects("Skill_HM");
    ...
    public void WeaponSkill(PlayerWeapon Weapon)
    {
        ...
        Weapon.SkilPrefab.GetComponent<EffectSet>().enabled = true;
    ...
    {
        GameSoundManager.Instance.SetPlayerFx("hammer1", Weapon.CurrentWeapon.gameObject.transform);
    }

    public void SAtkSound(PlayerWeapon Weapon)
    {
        GameSoundManager.Instance.SetPlayerFx("hammer2", Weapon.CurrentWeapon.gameObject.transform);
    }
}
